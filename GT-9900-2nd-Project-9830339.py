...
def colorGraph(graph):
	...
	for X, _ in enumerate(graph):
		if _DEBUG: print()
		changed = True
		while changed:  # max is num of adjs
			changed = False
			...
			f = F[0]
			l = F[-1]
			l_freeColors = freeColorsOf(graph, l)
			d = l_freeColors[0]  # or random
			X_freeColors = freeColorsOf(graph, X, [d])  # except color d already picked
			c = X_freeColors[0]  # or random
			...
			for vPos, v in enumerate(F):
				if d in freeColorsOf(graph, v):
					wPos, w = vPos, v
					break
			if w is None:  # impossible
				continue

			# subFan= <f,l>and<f,w> = <f,w>
			subFan = F[:wPos + 1]
